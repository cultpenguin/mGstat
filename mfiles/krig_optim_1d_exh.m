...
hold on
plot(options.pos_known_all,options.val_known_all(:,1),'k.','MarkerSize',10)
...
hold on;
p1=plot(pos_est,[d_be d_be+sqrt(v_be) d_be-sqrt(v_be)],'k-');
p2=plot(pos_est,[d_L d_L+sqrt(v_L) d_L-sqrt(v_L)],'r-');
p3=plot(pos_est,[d_L2 d_L2+sqrt(v_L2) d_L2-sqrt(v_L2)],'b--');
hold off
