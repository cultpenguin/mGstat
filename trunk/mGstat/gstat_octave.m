...
if isstruct(G)==1,
    gstat_filename=write_gstat_par_octave(G);
else
    gstat_filename=G;
    G=read_gstat_par_octave(gstat_filename);
end
...
if isfield(G,'set');
    ...
    write_gstat_par_octave(G);
    G=read_gstat_par_octave(G.mgstat.parfile);
end
