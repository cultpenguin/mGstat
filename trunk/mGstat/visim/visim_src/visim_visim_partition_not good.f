...
c     idbg             integer debugging level (0=none,2=normal,4=ctableous)
...
c-----------------------------------------------------------------------
      ...
      real meantmh,stdtmh 
      integer do_partition, partition_size
      integer l1, l2, l
      real usev_org(MAXVOLS),nusev_org
      real gmean_org, gvar_org
...
c 	SET THE GLOBAL VARIANCE TO THE MAXIMUM COVARIANCE (VERSION 1.5->)
      gvar = cbb
...
c
c  Partition of Data into SUBSETS
c  if data the uncertainty of data observations are unocrrelated (offfiag(Cd)=0)
c  then the least squares inversion can be solved by partitioning data into 
c  subsets (using subsets as small as one data), following for example
c  Tarantola's 2005 book, chapter 6.18.
c  Using this one can ensure that compuation time grows linearly with the number of
c  conditioning data !
c                 
...
c			STORE ORG VALUES		
			...
			nusev=j
c	
c	WE ARE HERE : WHY DOES THE KRIGING VARIANCE NOT GET SMALLER ?
c       HOW DO WE PASS THE GLOBAL VARIANCE ?
c
c	
c		        write(*,*) '---PART0>',gmean,cmean,cstdev,gvar,cbb
                        call krige_volume(ix,iy,iz,xx,yy,zz,lktype,
     +                    gmean,cmean,cstdev,index)
		        write(*,*) '---PART>',gmean,cmean,cstdev,gvar,cbb
c		          gvar = cstdev*cstdev
c		          cbb = cstdev*cstdev
		          gmean = cmean
		    
			enddo
		
c			RESTORE ORG VALUES		
		       	do k=1,nusev_org
		      		usev(k)=usev_org(k)
         		enddo		
         		gmean=gmean_org;
...
c                     write(*,*) 'in=',in
c                     lktype = ktype
c                     if(ktype.eq.1.and.(nclose+ncnode).lt.4)lktype=0

 
                     call krige_volume(ix,iy,iz,xx,yy,zz,lktype,
     +                    gmean,cmean,cstdev,index)
c		     write(*,*) '---KRIG>',gmean,cmean,cstdev,gvar,cbb
