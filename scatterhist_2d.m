...
if plot_1d==1;
    ...
    Px=hist(x_acc,x);Px=Px/max(Px);
    Py=hist(y_acc,y);Py=Py/max(Py);
    
    g=1;
    %g=0.5;
    plot(x,y0+zeros(1,nx)+gain*ly.*Px,'-','color',[1 1 1].*g,'LineWidth',2)
    plot(x0+zeros(1,ny)+gain*lx.*Py,y,'-','color',[1 1 1].*g,'LineWidth',2)
    hold off
end
